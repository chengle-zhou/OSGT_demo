...
%
% irntv --  Compute the minimum of a generalised TV functional 计算广义TV函数的最小值
%
...
%  
  ...
  % Check Input/Output general parameters options检查输入/输出常规参数选项
  [dflag, InputDims, pars.sbstflg, pars.lambda_ini] = check_irntv_options(nargout, S, KC, pars);
  

  % Set up status display设置状态显示
  iterdispopen(dflag);      


  % choose Regularization weighting scheme (valid for all cases)选择正则化加权方案（在所有情况下均有效
  [fR fR_var4] = choose_Regularization_weighting_scheme(pars);


  % choose Fidelity weighting scheme (no need for POISSON or SPECKLE cases)选择保真度加权方案（不需要POISSON或SPECKLE情况）
...
% Choose the weighting scheme for the Fidelity term (valid for l1/l2-TV)
  ...
  switch(pars.weight_scheme) 
    ...
    case{NMP_WEIGHTS_THRESHOLD}
      ...
      if( isempty(pars.adapt_epsF) || pars.adapt_epsF == 0 )    % No adaptation of the Fidelity wieghts不调整保真度权重
...
% Choose the weighting scheme for the Regularization term (valid for all schemes)
  ...
  switch(pars.weight_scheme) 
    ...
    % Apply threshold to weights将阈值应用于权重
    case{NMP_WEIGHTS_THRESHOLD}
